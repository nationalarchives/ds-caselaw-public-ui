const DETAILS_SELECTOR = "[data-stateful-details]";
const CLOSED_TITLE_SELECTOR = "[data-stateful-details-close-title]";
export const COOKIE_NAME = "cookie_preferences_set";
const HIDE_CLOSED_TITLE_AFTER_MS = 6000;

export interface StatefulDetailsOptions {
  closedTitleSelector?: string;
  cookieName?: string;
  hideClosedTitleAfterMs?: number;
  cookieString?: string | null;
}

export function getCookie(
  name: string,
  cookieString: string | null = document.cookie,
): string | null {
  const cookies = cookieString ? cookieString.split("; ") : [];
  for (const c of cookies) {
    const idx = c.indexOf("=");
    const key =
      idx >= 0 ? decodeURIComponent(c.slice(0, idx)) : decodeURIComponent(c);

    if (key === name) {
      const val = idx >= 0 ? c.slice(idx + 1) : "";
      return decodeURIComponent(val);
    }
  }
  return null;
}

export function initStatefulDetails(
  details: HTMLDetailsElement | null,
  options: StatefulDetailsOptions = {},
): () => void {
  const {
    closedTitleSelector = CLOSED_TITLE_SELECTOR,
    cookieName = COOKIE_NAME,
    hideClosedTitleAfterMs = HIDE_CLOSED_TITLE_AFTER_MS,
    cookieString,
  } = options;

  if (!details) return () => {};

  const closedTitle = details.querySelector<HTMLElement>(closedTitleSelector);
  const summary = details.querySelector("summary");

  const isCookieSet =
    getCookie(cookieName, cookieString ?? document.cookie) === "true";

  let hideTimerId: number | undefined;
  let userHasInteracted = false;

  const showClosedTitle = () => {
    if (!closedTitle) return;

    closedTitle.hidden = false;

    window.clearTimeout(hideTimerId);
    hideTimerId = window.setTimeout(() => {
      closedTitle.hidden = true;
      hideTimerId = undefined;
    }, hideClosedTitleAfterMs);
  };

  if (closedTitle) closedTitle.hidden = true;

  details.open = !isCookieSet;

  const onToggle = () => {
    if (!userHasInteracted) return;

    if (details.open) {
      if (closedTitle) closedTitle.hidden = true;
      window.clearTimeout(hideTimerId);
      hideTimerId = undefined;
    } else {
      if (!isCookieSet) showClosedTitle();
    }
  };

  const onSummaryClick = () => {
    userHasInteracted = true;
  };

  details.addEventListener("toggle", onToggle);
  if (summary) summary.addEventListener("click", onSummaryClick);

  return () => {
    details.removeEventListener("toggle", onToggle);
    if (summary) summary.removeEventListener("click", onSummaryClick);
    window.clearTimeout(hideTimerId);
    hideTimerId = undefined;
  };
}

document.addEventListener("DOMContentLoaded", () => {
  const detailsList = Array.from(
    document.querySelectorAll<HTMLDetailsElement>(DETAILS_SELECTOR),
  );

  return detailsList.map((details) => initStatefulDetails(details, {}));
});
