...
def _normalise_request_body(body) -> bytes:
    if body is None:
        return b""
...
_CANONICAL_MARKLOGIC_USER = "cassette_user"


def _normalise_user_in_request_body(body: bytes) -> bytes:
    if not body:
        return body
    try:
        ...
        if "vars" not in params:
            return body
        vars_dict = json.loads(params["vars"])
...
def _body_matcher(r1, r2):  # noqa: C901
    body1 = _normalise_user_in_request_body(_normalise_request_body(getattr(r1, "body", None)))
    body2 = _normalise_user_in_request_body(_normalise_request_body(getattr(r2, "body", None)))
    ...
    if uri1 != uri2:
        if body1 != body2:
            raise AssertionError(f"Body mismatch for different URIs: {body1!r} != {body2!r}")
        return
    ...
    if uri1.endswith("/LATEST/eval"):
        ...
        if form1.keys() != form2.keys():
            raise AssertionError(f"Form keys mismatch: {form1.keys()} != {form2.keys()}")
        ...
        for key in form1:
            ...
            if key == "vars":
                if _normalise_json_string(v1) != _normalise_json_string(v2):
                    raise AssertionError(f"vars mismatch: {v1!r} != {v2!r}")
            else:
                if v1.strip() != v2.strip():
                    raise AssertionError(f"Form field {key!r} mismatch: {v1!r} != {v2!r}")

        return

    if body1 != body2:
        raise AssertionError(f"Body mismatch: {body1!r} != {body2!r}")
...
def _before_record_request(request):
    ...
    request.body = _normalise_user_in_request_body(request.body or b"")
    return request
